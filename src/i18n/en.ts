...
export const messages: Record<string, string> = {
  ...
  'cmd.meeting.no_session': 'There is no usable meeting-preparation session. Run this command in a regular group chat.',
  'cmd.meeting.owner_only': 'Only the bot owner can configure meeting preparation groups.',
  'cmd.meeting.invalid': 'The meeting link or meeting number is invalid.',
  'cmd.meeting.usage': 'Usage:\n• /meeting prepare <meeting link or number> [--qa auto|off] — bind the current regular group as the meeting preparation chat\n• /meeting status [meeting number] — list preparation records\n• /meeting off [meeting number|all] — remove the current, specified, or every binding',
  'cmd.meeting.none': 'No meeting preparation record was found.',
  'cmd.meeting.status_title': 'Meeting preparation records:',
  'cmd.meeting.stopped': 'Removed {count} meeting preparation binding(s).',
  'cmd.meeting.need_group_chat': 'Run `/meeting prepare` in a regular group chat-scope session. Topic sessions cannot share the same Agent session with the live meeting stream.',
  'cmd.meeting.prepared': 'The meeting preparation chat is ready.\nMeeting number: `{meetingNo}`\nAutomatic Q&A: `{qaMode}`\nAfter the meeting starts and the Bot is invited, Botmux will reuse this group and its current Agent session. Add background here and run `/watch-comment <document link> --all` to pre-read and watch the presentation document.',
  'cmd.meeting.project_bound': '📂 Reusing the project bound to this session: {dir}',
  'cmd.meeting.project_optional': 'No project directory is bound to this session. To let meeting Q&A use local code, send `/cd <project directory>`; otherwise answers will use this chat, watched documents, and meeting captions only.',
  'cmd.meeting.replaced': 'Note: this group was also bound to meeting `{meetingNo}`. The old record remains; remove it with `/meeting off {meetingNo}` if needed.',
  ...
  'help.meeting': '/meeting prepare <meeting link or number> - Use the current regular group as a preparation chat and reuse the same Agent session during the meeting',
