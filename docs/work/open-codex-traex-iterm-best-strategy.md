...
两个规划子 agent 独立调研后结论一致：不要继续依赖 `codex://`、`trae://`、`traex://` 或 `multi_url` 来触发本地 CLI。最佳策略是新增飞书卡片 callback action，由 botmux daemon 在本机通过 iTerm AppleScript 打开终端，并从服务端 session 状态重新构造命令。
...
- 仅对 `cliId=codex` / `cliId=traex` 展示本地按钮，文案体现本机 iTerm 语义，例如“iTerm 打开 Codex”/“iTerm 打开 TRAE”。
...
- 终端承载固定 iTerm，不由 `cliId=traex` 推导 Trae terminal。

## 命令构造优先级

第一版优先保证点击后稳定打开 iTerm 并恢复对应 CLI。
...
2. 否则若 active session 是 botmux-managed tmux backend，打开 iTerm 执行 `tmux attach-session -t bmx-<sessionId-prefix>`，直连当前 live pane，避免复制出第二个 CLI。
...
5. zellij/herdr 第一版不做 iTerm 直连，包含 managed active session 和 adopt session，均返回明确 toast。
...
- `src/im/lark/card-handler.ts`
  ...
  - 新增 action 分支，调用本地 opener，并返回 success/error toast。
- `src/services/local-cli-opener.ts`
  - 新增命令构造、shell/AppleScript escape、iTerm 启动逻辑。
...
- 点击“打开 TRAE”必须打开 iTerm，不能打开 Trae terminal。
- 点击“打开 Codex”必须有 iTerm 动作；失败时有明确 toast，不能看起来没反应。
- 非授权用户不能触发本机命令。
...
- 不把 iTerm 失败 fallback 到 Trae terminal。

## 实施记录

2026-07-09:

- 新增 `src/services/local-cli-opener.ts`，本地打开仅白名单支持 `codex` / `traex`，通过 `/usr/bin/osascript` 驱动 iTerm。
